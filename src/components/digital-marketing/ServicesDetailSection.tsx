...
const services = [
  ...
  },
  {
    ...
    description: "From brand identity systems to daily social media creatives — our in-house designers create visuals that communicate your story and stand out.",
  ...
  },
  {
    ...
    description: "We build automated email sequences and content funnels that nurture leads and convert subscribers into loyal customers.",
  ...
  <section className="py-8 md:py-16 lg:py-24 bg-muted/20">
    <div className="container mx-auto px-4">
      <div className="text-center max-w-3xl mx-auto mb-6 md:mb-14">
        <span className="inline-block px-3 py-1 bg-brand/10 text-brand rounded-full text-xs sm:text-sm font-medium mb-3 border border-brand/20">Digital Marketing Services in Detail</span>
        ...
        <p className="text-muted-foreground text-xs sm:text-sm md:text-lg">A closer look at each service and how it drives measurable growth for Bangalore businesses.</p>
      </div>
          ...
          <div key={i} className="bg-white border border-border/50 rounded-xl p-4 sm:p-6 md:p-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2 mb-4">
              {service.features.map((feature, fi) => (
                <div key={fi} className="flex items-center gap-1.5 text-[11px] sm:text-sm text-foreground font-medium">
                  <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-brand flex-shrink-0" />
                  {feature}
                </div>
              ))}
            </div>
