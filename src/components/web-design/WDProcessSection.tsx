...
const steps = [
  { icon: Search, number: "01", title: "Discovery & Wireframing", description: "We start by understanding your business goals, target audience, and market landscape. Detailed sitemaps and wireframes are prepared to define the structure before the design begins." },
  { icon: Layers, number: "02", title: "UI/UX Design", description: "High-quality design mockups are created using your brand colours, typography, and visual identity. Every page is reviewed and approved before development starts." },
  { icon: Rocket, number: "03", title: "Development & Quality Testing", description: "Clean, responsive development with complete testing across all devices and browsers. Performance optimization and SEO fundamentals are included." },
  { icon: TrendingUp, number: "04", title: "Launch & Optimization", description: "Website deployment, analytics configuration, tracking setup, and continuous optimization based on real user behaviour and performance data." },
];
    ...
    <section ref={sectionRef} className="py-8 md:py-14 lg:py-20 bg-muted/30 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative">
        <div className={`text-center max-w-3xl mx-auto mb-6 md:mb-12 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">From Concept to Launch in Four Clear Steps</h2>
          <p className="text-lg text-muted-foreground">A structured and transparent development process that keeps you informed at every stage of your website project.</p>
        </div>
