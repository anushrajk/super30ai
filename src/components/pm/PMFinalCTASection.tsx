    ...
    <section ref={ref} className={`py-8 md:py-14 lg:py-20 relative bg-background overflow-hidden transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      <div className="container mx-auto px-4 relative">
        <div className="max-w-3xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-background border border-border px-4 py-1.5 rounded-full mb-6">
            <Target className="w-4 h-4 text-foreground" />
            <span className="text-foreground text-sm font-medium">Free Ads Strategy Session</span>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-8">
            {benefits.map((item, i) => (
              <div key={i} className="flex items-center gap-2 text-foreground bg-background border border-border rounded-full px-4 py-2">
