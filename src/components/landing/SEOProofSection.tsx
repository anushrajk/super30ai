...
interface SEOProofSectionProps {
  layout?: "carousel" | "grid";
}

export const SEOProofSection = ({ layout = "carousel" }: SEOProofSectionProps) => {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 max-w-4xl mx-auto mb-12">
            {highlights.map((highlight) => (
              <div key={highlight.label} className="rounded-2xl border border-border/60 bg-background p-4 md:p-6 text-center">
                <highlight.icon className="w-5 h-5 text-brand mx-auto mb-2" />
                <div className="text-2xl md:text-3xl font-bold text-foreground">{highlight.value}</div>
                <p className="text-[11px] md:text-xs text-muted-foreground mt-1">{highlight.label}</p>
              </div>
            ))}
          </div>
              ...
              >
                <div className="relative bg-muted/40 border-b border-border/40 p-3">
                  <div className="rounded-xl overflow-hidden border border-border/50 bg-white shadow-sm">
                    ...
                    <div className="aspect-[16/9] bg-white flex items-center justify-center">
                      <img
                        src={report.image}
                        alt={`${report.name} SEO performance report`}
                        loading="lazy"
                ...
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-4">
                    <img src={report.logo} alt={report.name} className="w-9 h-9 rounded-lg object-contain bg-muted p-1" loading="lazy" />
                    <h3 className="font-bold text-sm text-foreground leading-tight">{report.name}</h3>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {report.metrics.map((metric) => (
                      <div key={metric.label} className="rounded-xl bg-muted/50 py-2.5 px-1 text-center">
                        <div className="text-base font-bold text-brand">{metric.value}</div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {wins.map((win) => (
              ...
              >
                <img src={win.logo} alt={win.name} className="w-10 h-10 rounded-lg object-contain bg-muted p-1 shrink-0" loading="lazy" />
                <div className="min-w-0">
                  <div className="text-lg font-bold text-brand leading-none">{win.value}</div>
                  <p className="text-[11px] text-muted-foreground truncate">{win.label}</p>
                  <p className="text-xs font-semibold text-foreground truncate mt-0.5">{win.name}</p>
                </div>
              </Link>
            ))}
            <Link
